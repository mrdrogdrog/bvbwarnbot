...
function determineReason(match: ParsedApiMatch): Reason | null {
  const isGermanTeam = match.team1.teamName === TEAM_NAME || match.team2.teamName === TEAM_NAME
  const isInDortmund = match.location.locationID === LOCATION_ID_DORTMUND || match.location.locationCity.toLowerCase().includes(LOCATION_NAME_DORTMUND.toLowerCase())
  if (isGermanTeam && isInDortmund) {
    return "emGermanTeamInDortmund"
  } else if (isGermanTeam) {
    return "emGermanTeam";
  } else if (isInDortmund) {
